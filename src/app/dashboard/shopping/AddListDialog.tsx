...
export function AddListDialog({ open, onClose, onCreate }: AddListDialogProps) {
  const [name, setName] = useState("Weekly Shopping");
          ...
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <p className="text-[10px] sm:text-xs text-muted-foreground">
                Start a fresh shopping list with a custom name.
              </p>
            </div>
            <Button
              type="button"
          ...
          <form onSubmit={handleSubmit} className="space-y-2 sm:space-y-4">
            <div className="space-y-1">
              ...
              <input
                ...
                placeholder="Weekly Shopping, Diwali Haul..."
                value={name}
            ...
            <Button
              type="submit"
              disabled={!name.trim()}
              className="w-full h-8 sm:h-10 bg-[#6366F1] hover:bg-[#4F46E5] text-white shadow-lg shadow-indigo-500/40 flex items-center justify-center gap-2 text-[10px] sm:text-sm"
            >
